...
# ============================
# CONFIGURAZIONE V21.20 - FULL SNIPER (COMPLETE ARCHITECTURE)
# ============================
...
st.set_page_config(page_title="ARAB SNIPER V21.20 - FULL SNIPER", layout="wide")
...
# ============================
# INITIALIZATION & DATABASE ROLLING
# ============================
    ...
    if os.path.exists(db_path):
        try:
            with open(db_path, "r") as f:
                ...
                # Mantieni solo i match da oggi in poi (Rolling)
...
# ============================
# MOTORE STATISTICO (HUNGER & ELITE)
# ============================
team_stats_cache = {}
...
def get_stats(session, tid):
    ...
    try:
        ...
        fx = rx.get("response", [])
        if not fx: return {"ht_std": False, "is_elite": False, "vul_5": 0.0, "o25_8": 0.0, "gg_8": 0.0, "gght_stat_ok": False}
        ...
        for idx, f in enumerate(fx):
            ...
            gh_ht, ga_ht = score_ht.get("home") or 0, score_ht.get("away") or 0
            if (gh_ht + ga_ht) >= 1: ht_c += 1
            if gh_ht > 0 and ga_ht > 0: gght_hist += 1
            is_h = (f["teams"]["home"]["id"] == tid)
            if ((f["goals"]["home"] or 0) + (f["goals"]["away"] or 0)) >= 3: o25_c += 1
            if (f["goals"]["home"] or 0) > 0 and (f["goals"]["away"] or 0) > 0: gg_c += 1
            if idx < 5:
        ...
        return {"ht_std": False, "is_elite": False, "vul_5": 0.0, "o25_8": 0.0, "gg_8": 0.0, "gght_stat_ok": False}

# ============================
# ESTRAZIONE MERCATI (ANTI-NOISE)
# ============================
...
def execute_scan(session, fixtures, snap_mem, excluded, min_rating_val):
    ...
    for i, m in enumerate(filtered):
        ...
        try:
            ...
            avg_vul = (s_h["vul_5"] + s_a["vul_5"]) / 2
            is_boost = (HT_OK and (1.60 <= mk["o25"] <= 2.15) and (1.20 <= mk["o05ht"] <= 1.55) and (avg_vul >= 0.60 or f_stats["vul_5"] >= 0.80) and f_stats["o25_8"] >= 0.625)
...
def run_scan(is_snap):
    with requests.Session() as s:
        try:
            ...
            # Update Database avoiding duplicates
...
if st.session_state["scan_results"]:
    ...
    if not df_view.empty:
        ...
        st.markdown("---")
        st.download_button("💾 CSV AUDITOR (DB COMPLETO)", df.to_csv(index=False).encode('utf-8'), f"audit_full_{target_dates[0]}.csv")
