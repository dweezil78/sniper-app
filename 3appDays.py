        ...
        try:
            ...
            if rating >= min_rating_val:
                results.append({
                    ...
                    "Quota GG1T": f"{mk['gg_ht']:.2f}", "Info": f"[{'|'.join(det)}]", "Rating": rating, "Gold": "✅" if (1.40 <= mk[fav_side] <= 2.10) else "❌",
                    "Is_Gold_Bool": (1.40 <= mk[fav_side] <= 2.10), "O25_OK": (1.70 <= mk["o25"] < 2.10), "Is_Super_Red": is_super_red
                })
        except: continue
    return results
