...
st.set_page_config(page_title="ARAB SNIPER V15.36 - GOLD MASTER", layout="wide")
...
def calculate_rating(fid, q1, qx, q2, o25, o05ht, snap_data, max_q_gold, trap_limit, inv_margin):
    ...
    if 0 < current_fav < trap_limit:
        if fid_s in snap_data:
            ...
            if old_q >= trap_limit and (old_q - current_fav) >= 0.10:
                into_trap = True
...
# Mostra lo stato dello Snapshot
...
with c1:
    if st.button("📌 SALVA/AGGIORNA SNAPSHOT"):
        with requests.Session() as s:
            ...
            if not valid_fx:
                st.warning("Nessun match NS trovato per oggi.")
                st.stop()
        ...
        for i, m in enumerate(fixtures):
            ...
            try:
                ...
                if rating >= min_rating:
                    ...
                    advice = ""
                    if is_gold:
                        if "DRY" in det: advice = "🔥 TARGET: 0.5 HT LIVE"
                        elif "HT" in det: advice = "🔥 TARGET: 0.5 HT / 2.5 FT"
                    
                    match_name = f"{m['teams']['home']['name']} - {m['teams']['away']['name']}"
                    if into_trap: match_name = f"{match_name} *"
                    
                    # Nome Lega con Nazione per scelta rapida
                    lega_info = f"{m['league']['name']} ({m['league']['country']})"
                    
                    results.append({"Ora": m["fixture"]["date"][11:16], "Lega": lega_info, "Match": match_name, "1X2": f"{mk['q1']:.2f}|{mk['qx']:.2f}|{mk['q2']:.2f}", "O2.5": f"{mk['o25']:.2f}", "O0.5HT": f"{mk['o05ht']:.2f}" if mk.get("o05ht", 0) > 0 else "N/D", "Rating": rating, "Info": f"[{'|'.join(det)}]", "Advice": advice, "Is_Gold": is_gold, "Drop_Inv": d_html, "Drop_Inv_Text": d_text, "Fixture_ID": m["fixture"]["id"]})
...
if st.session_state["scan_results"]:
    ...
    if res["data"]:
        ...
        # FILTRO VISIVO ISTANTANEO: agisce sui dati già scansionati
        if only_gold_ui:
            df_display = df_d[df_d["Is_Gold"] == True].copy()
        else:
        ...
        if not df_display.empty:
            ...
            st.info("Nessun match Gold trovato con l'attuale filtro visivo.")
        ...
        with col_dl2:
            html_table = df_d.to_html(escape=False, index=False)
            html_styled = f"<html><body><style>table{{border-collapse:collapse;width:100%;font-family:Arial;}} th,td{{border:1px solid #ddd;padding:8px;text-align:center;}} th{{background-color:#f2f2f2;}}</style>{html_table}</body></html>"
            st.download_button("🌐 SCARICA REPORT HTML", data=html_styled.encode('utf-8'), file_name=f"report_{oggi}.html", mime="text/html")
        with col_dl3:
            if os.path.exists(LOG_CSV):
