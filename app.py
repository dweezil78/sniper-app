...
def apply_custom_css():
    st.markdown("""
        <style>
            ...
            /* Testo NERO forzato per le righe chiare per visibilità totale */
...
ALWAYS_ALLOW_COUNTRIES = set(["Australia","Japan","Argentina","Brazil","USA","Mexico","Italy"])
...
def save_json_snapshot(data_dict: Dict[int, Dict]):
    ...
    payload = {"date": oggi, "odds": data_dict}
    with open(JSON_FILE, "w") as f:
        json.dump(payload, f)

def load_json_snapshot() -> Tuple[Optional[str], Dict[int, Dict]]:
...
def fetch_odds(fixture_id: int):
    ...
    for b in bets:
        if b["id"] == 1: # 1X2
            q1, qx, q2 = float(b["values"][0]["odd"]), float(b["values"][1]["odd"]), float(b["values"][2]["odd"])
        if b["id"] == 5: # Over 2.5
            o25 = float(next((x["odd"] for x in b["values"] if x["value"] == "Over 2.5"), 0))
    return q1, qx, q2, o25

@st.cache_data(ttl=3600)
def get_ht_rate(team_id: int):
    url = f"https://{HOST}/fixtures"
    r = requests.get(url, headers=HEADERS, params={"team": team_id, "last": 5, "status": "FT"}).json()
    fx = r.get("response", [])
    if not fx: return 0.0
    ht_goals = sum([1 for f in fx if (f["score"]["halftime"]["home"] or 0) + (f["score"]["halftime"]["away"] or 0) >= 1])
...
def calculate_rating(fid, q1, q2, o25, h_ht, a_ht, snap_data):
    ...
    if str(fid) in snap_data:
        ...
        # Capisce chi è il favorito
        is_h = q1 < q2
        old_fav = old["q1"] if is_h else old["q2"]
        ...
        # Inversione (20 pt)
        if (old["q1"] < old["q2"] and q2 < q1) or (old["q2"] < old["q1"] and q1 < q2):
            sc += 20; det.append("INVERSIONE ↔️ (+20)")
            drop_msg = "🔄 INVERSIONE"

    # 2. HT GOAL RATE (20 pt)
