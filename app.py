...
st.set_page_config(page_title="ARAB SNIPER V15.45 - GOLD MASTER", layout="wide")
...
with st.sidebar.expander("📖 LEGENDA SIMBOLI", expanded=True):
    st.markdown("""
    **👑 GOLD (Sweet Spot)**: Favorita 1.40-1.95.
    **🔄 INV**: Cambio favorito nel mercato.
    **📉 Drop**: Crollo netto della quota.
    **(*) Asterisco**: Crollo in zona Trap (<1.40).
    **🔥 TARGET**: Consiglio operativo (HT/DRY).
    """)
        ...
        with requests.Session() as s:
            try:
                ...
                for i, m in enumerate(valid_fx):
                    ...
                    status_text_snap.text(f"Snapshotting {i+1}/{len(valid_fx)}: {m['teams']['home']['name']}...")
...
if st.button("🚀 AVVIA SCANSIONE MATCH"):
    ...
    with requests.Session() as s:
        try:
            ...
            if not fixtures: st.warning("Nessun match con i filtri attuali."); st.stop()
            
            results, pb = [], st.progress(0)
            status_text_scan = st.empty()
            for i, m in enumerate(fixtures):
                pb.progress((i+1)/len(fixtures))
                status_text_scan.text(f"Analisi {i+1}/{len(fixtures)}: {m['teams']['home']['name']}...")
                try:
                    mk = extract_markets_pro(api_get(s, "odds", {"fixture": m["fixture"]["id"]}))
                    ...
                    if rating >= min_rating:
                        ...
                        results.append({
                            ...
                            "O2.5": f"{mk['o25']:.2f}",
                            "O0.5HT": f"{mk['o05ht']:.2f}" if mk["o05ht"] > 0 else "N/D",
                            "Rating": rating,
                            "Info": f"[{'|'.join(det)}]",
                            "Is_Gold": is_gold,
                            "Advice": advice,
...
if st.session_state["scan_results"]:
    ...
    if not df_show.empty:
        ...
        cols_to_show = ["Ora", "Lega", "Match_Disp", "1X2", "O2.5", "O0.5HT", "Rating", "Info"]
